        ...
        dispatcher.register(CommandManager.literal("polymc-showcase")
                .then(commandWithOptionalTarget("query", (context, target) -> {
                    if (((PlayerDuck)target).getIsUsingPolyMc()) {
                        context.getSource().sendFeedback(() -> Text.literal("PolyMc is enabled for ").append(target.getDisplayName()), false);
                    } else {
                        context.getSource().sendFeedback(() -> Text.literal("PolyMc is disabled for ").append(target.getDisplayName()), false);
                    }
